...
QUEUE_URL=$(aws sqs get-queue-url --queue-name shopstream-orders-stream --region $REGION --query 'QueueUrl' --output text 2>/dev/null) || QUEUE_URL=""

if [ -z "$QUEUE_URL" ]; then
    echo "Creating queue: shopstream-orders-stream"
    
    QUEUE_URL=$(aws sqs create-queue \
      --queue-name shopstream-orders-stream \
...
if aws lambda get-function --function-name shopstream-orders-stream &>/dev/null; then
    echo "✓ Function already exists: shopstream-orders-stream"
else
...
def lambda_handler(event, context):
    ...
    """
    bucket = event.get('bucket_name', 'shopstream-datalake-463183325212-us-east-1-an')
    records = []
    ...
    if records:
        ...
        key = f"raw/streaming/orders/year=!{timestamp:yyyy}/month=!{timestamp:MM}/batch-{batch_time}.csv"
...
EOFHANDLER
    ...
    aws lambda create-function \
      --function-name shopstream-orders-stream \
      --runtime python3.12 \
...
echo "✓ Lambda Consumer:        shopstream-orders-stream"
